...
\begin{document}
\title{A competition on formal methods for robotics\\ {\tiny version~0.0.4}}
...
\begin{itemize}
\item 0 if the trial is skipped;
\item -1 if the trial is attempted but a collision with an obstacle occurs
or there is some goal region not reached;
\item scaled by:
\[
\frac{(\#\text{output dimensions})*(\#\text{integrators})}{(\text{time between
receiving problem instances and beginning execution})}.
\]
\end{itemize}

The number of trials, and parameters like the number of output dimensions 
will be the same for each team/controller.
...
\item~[NOT USED 2016] stopping at intersections, which are treated as all-ways stops, and then
...
\begin{enumerate}
\item -1 if a collision occurs between the ego vehicle and any other
vehicle;
\item otherwise,
\[
\frac{(\text{\#e-agents})*(\text{\#road grid rows})*(\text{\#cols})*X}{\max\{1, (\text{\#seconds ego vehicle not on
some road})\}},
\]
...
\bigskip
The number of trials, and parameters like the size of the road network
will be the same for each team/controller.

...
\end{split}
\end{equation}
where for time $t$, each $x_{i}(t)\in \mathbb{R}^n$ for $i=1,\ldots,m$, and $x(t)=\left( x_{1}(t) , \ldots , x_{m}(t)\right)\in
\mathbb{R}^{nm}$.  That
is, \eqref{eq:cintegdyn} is a linear control system of dimension $mn$, and is equivalent to the
dynamical system in \eqref{eq:minteg}, where control input is applied as the $m$-th derivative of an $n$-dimensional
variable $q$. Notably, $m=2$ is known as the ``double integrator'' case, and if $n \leq 3$ then $q$ may be referred to as the ``position''.
...
In general, the resulting linear time-invariant system will thus be of dimension $mn$.
